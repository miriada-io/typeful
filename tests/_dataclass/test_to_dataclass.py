...
def test_to_dataclass__simple():
    ...
    assert (
            to_dataclass(dict(integer=123, string='test'), ExampleSimpleDataclass)
            ==
            ExampleSimpleDataclass(integer=123, string='test')
    )
...
def test_to_dataclass__complex():
    ...
    assert (
            to_dataclass(dict(limit=123), ExampleComplexDataclass)
            ==
            ExampleComplexDataclass(limit=123)
    )
...
def test_to_dataclass__nested():
    ...
    assert (
            to_dataclass({'first': {"integer": 123}, 'second': {"integer": 678}}, ExampleNestedDataclass)
            ==
            ExampleNestedDataclass(
                first=ExampleDelegateDataclass(integer=123),
                second=ExampleDelegateDataclass(integer=678)
            )
    )
...
def test_to_dataclass__complex_nested():
    ...
    assert (
            ...
            ExampleNestedDataclass(
                dict_={
                    123: ExampleValueDataclass('test'),
                    234: ExampleValueDataclass('passed')
                },
                list_=[ExampleValueDataclass("12"), ExampleValueDataclass("23")],
                dict_of_sets={
                    0: set(),
                    1: {ExampleKeyDataclass(11)},
                    2: {ExampleKeyDataclass(22), ExampleKeyDataclass(3)}
                },
            )
    )
...
def test_to_dataclass__annotated():
    ...
    assert (
            to_dataclass({'first': {"integer": 123}, 'second': {"integer": 678}},
                         typing.Annotated[ExampleNestedDataclass, 'Target type'])
            ==
            ExampleNestedDataclass(
                first=ExampleDelegateDataclass(integer=123),
                second=ExampleDelegateDataclass(integer=678)
...
@pytest.mark.parametrize(['payload', 'target_type', 'expected'], [
